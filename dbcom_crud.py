...
'''导入自己编写的dbcom模块
'''
from dbcom import Session,Departments,Employees,Salary
...
#######################################3
'增加部门(添加实例)'
# hr = Departments(dep_id=1, dep_name='人事部')  #添加六条实例
...
# session.add_all([hr, ops, dev, qa, market, sales]) #根据实例向表中插入数据
######################################
...
#     emp_name='关羽',
#     birrh_date='1973-5-4',
#     email='gy@qq.com',dep_id=2
# )
# zf = Employees(
#     emp_id=3,
#     emp_name='张飞',
#     birrh_date='1974-10-1',
#     email='zf@tedu.cn',dep_id=3
# )
# zy = Employees(
#     emp_id=4,
#     emp_name='赵云',
...
# qset2 = session.query(Employees.dep_id,Employees.emp_name,\
# Employees.emp_id,Employees.email)
# print(qset2.all())
####################################
'排序'
...
##############################33
'取切片'
# qset4 = session.query(Departments).order_by(Departments.dep_id)[3:6]
# for dep in qset4:
#     print(dep.dep_id,dep.dep_name)
###############################
'过滤'
...
'过滤后,再过滤'
# qset6 = session.query(Departments).filter(Departments.dep_id>2)\
...
# qset10 = session.query(Employees.emp_name,Departments.dep_id)\
#     .join(Departments)
# for data in qset10:
#     print(data)
...
'修改'
# qset12 = session.query(Departments).filter(
